...
def _day_label(day, index, month, year, today_tuple):
...
def _month_table(cal, month, year, today_tuple):
    table = Table(
    ...
    for weekdays in cal.monthdayscalendar(year, month):
        days = [_day_label(day, index, month, year, today_tuple) for index, day in enumerate(weekdays)]
        table.add_row(*days)
...
def print_calendar(year):
    ...
    tables = [_month_table(cal, month, year, today_tuple) for month in range(1, 13)]
    console = Console()
