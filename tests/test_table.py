...
def _make_table_console() -> Console:
    return Console(
        width=60,
        force_terminal=True,
        file=io.StringIO(),
...
def _render_table_width_sweep(console: Console, table: Table) -> None:
    for width in range(10, 60, 5):
        console.print(table, width=width)


def _render_table_justified(console: Console, table: Table) -> None:
    table.expand = False
...
def _render_table_with_rows(console: Console, table: Table) -> None:
...
def _render_table_style_variants(console: Console, table: Table) -> None:
...
def _render_table_width_constraints(console: Console, table: Table) -> None:
    table.width = 20
...
def render_tables():
    ...
    _render_table_width_sweep(console, table)
    _render_table_justified(console, table)
    assert table.row_count == 1

    _render_table_with_rows(console, table)
    _render_table_style_variants(console, table)
    _render_table_width_constraints(console, table)
