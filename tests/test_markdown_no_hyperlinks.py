...
def _replace_link_ids(render: str) -> str:
    return re_link_ids.sub("id=0;foo\x1b", render)


def _render_markdown(markdown) -> str:
    from rich._console_entry import Console

    console = Console(
        width=100, file=io.StringIO(), color_system="truecolor", legacy_windows=False
    )
    console.print(markdown, no_wrap=False)
    return _replace_link_ids(console.file.getvalue())
...
def test_markdown_render():
    ...
    rendered_markdown = _render_markdown(markdown)
