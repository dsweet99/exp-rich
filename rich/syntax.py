...
from typing import TYPE_CHECKING, Any, Dict, Iterable, List, NamedTuple, Optional, Sequence, Set, Tuple, Type, Union
...
def _syntax_line_tokenize(lexer, code: str) -> Iterable[Tuple[Any, str]]:
...
def _syntax_spans_for_line_range(
    ...
    line_no = 0
    skip_until = (line_start - 1) if line_start else 0
    while line_no < skip_until:
        try:
    ...
    for token_type, token in tokens:
        yield (token, get_style(token_type))
        if token.endswith("\n"):
            line_no += 1
...
def _syntax_apply_stylized_ranges(text: Text, stylized_ranges, get_index) -> None:
    code = text.plain
    newlines_offsets = [
        0,
    ...
    for stylized_range in stylized_ranges:
        start = get_index(newlines_offsets, stylized_range.start)
        end = get_index(newlines_offsets, stylized_range.end)
...
def _syntax_wrapped_lines(syntax, console, line, render_options, background_style, transparent_background):
    if syntax.word_wrap:
...
def _syntax_yield_numbered_prefix(
    first: bool,
    line_no: int,
...
def _syntax_render_simple(syntax, console, options, text, code_width, ends_on_nl):
    if not ends_on_nl:
        text.remove_suffix("\n")
    style = (
        syntax._get_base_style()
        + syntax._theme.get_style_for_token(Comment)
        + Style(dim=True)
        + syntax.background_style
    )
    if syntax.indent_guides and (not options.ascii_only):
        text = text.with_indent_guides(syntax.tab_size, style=style)
        text.overflow = "crop"
...
def _syntax_render_numbered_line(
    syntax, console, line, render_options, background_style, transparent_background,
    numbers_column_width, number_style, highlight_number_style, highlight_line, line_no, line_pointer,
):
    wrapped_lines = _syntax_wrapped_lines(
        syntax, console, line, render_options, background_style, transparent_background
    )
    wrapped_line_left_pad = Segment(" " * numbers_column_width + " ", background_style)
    new_line = Segment("\n")
    for first, wrapped_line in loop_first(wrapped_lines):
        yield from _syntax_yield_numbered_prefix(
            first, line_no, numbers_column_width, highlight_line, line_pointer,
            highlight_number_style, number_style, wrapped_line_left_pad,
        )
        yield from wrapped_line
        yield new_line


def _syntax_render_lines(
    syntax,
    console,
    options,
    lines,
    ...
    numbers_column_width = syntax._numbers_column_width
    render_options = options.update(width=code_width)
    highlight_line = syntax.highlight_lines.__contains__
    line_pointer = '> ' if options.legacy_windows else '❱ '
    background_style, number_style, highlight_number_style = syntax._get_number_styles(console)
    transparent_background = syntax._get_base_style().transparent_background
    for line_no, line in enumerate(lines, syntax.start_line + line_offset):
        if syntax.line_numbers:
            yield from _syntax_render_numbered_line(
                syntax, console, line, render_options, background_style,
                transparent_background, numbers_column_width, number_style,
                highlight_number_style, highlight_line, line_no, line_pointer,
            )
        else:
            yield from _syntax_render_unnumbered_line(
                syntax, console, line, render_options, background_style,
                transparent_background,
            )


def _syntax_render_unnumbered_line(
    syntax, console, line, render_options, background_style, transparent_background,
):
    wrapped_lines = _syntax_wrapped_lines(
        syntax, console, line, render_options, background_style, transparent_background
...
class PygmentsSyntaxTheme(SyntaxTheme):
    ...
    def _style_from_pygments(self, token_type: TokenType) -> Style:
        try:
        ...
        return Style(color='#' + color if color else '#000000', bgcolor='#' + bgcolor if bgcolor else self._background_color, bold=pygments_style['bold'], italic=pygments_style['italic'], underline=pygments_style['underline'])
    ...
    def get_style_for_token(self, token_type: TokenType) -> Style:
        ...
        try:
            ...
            style = self._style_from_pygments(token_type)
...
class ANSISyntaxTheme(SyntaxTheme):
    ...
    def _lookup_style_for_token(self, token_type: TokenType) -> Style:
    ...
    def get_style_for_token(self, token_type: TokenType) -> Style:
        ...
        try:
            ...
            style = self._lookup_style_for_token(token_type)
            self._style_cache[token_type] = style
            return style
...
class Syntax:
    ...
    def highlight(self, code: str, line_range: Optional[Tuple[Optional[int], Optional[int]]]=None) -> Text:
        ...
        if lexer is None:
            ...
            tokens = iter(_syntax_line_tokenize(lexer, code))
            text.append_tokens(
                _syntax_spans_for_line_range(
                    tokens, line_start, line_end, _get_theme_style
                )
            )
        else:
            text.append_tokens(
        ...
        if self._stylized_ranges:
            _syntax_apply_stylized_ranges(
                text, self._stylized_ranges, _get_code_index_for_syntax_position
            )
        return text
    ...
    def _get_syntax(self, console: Console, options: ConsoleOptions) -> Iterable[Segment]:
        ...
        code_width = (options.max_width - self._numbers_column_width - 1 if self.line_numbers else options.max_width) - horizontal_padding if self.code_width is None else self.code_width
        code_width = max(0, code_width)
        ends_on_nl, processed_code = self._process_code(self.code)
        text = self.highlight(processed_code, self.line_range)
        if not self.line_numbers and (not self.word_wrap) and (not self.line_range):
            yield from _syntax_render_simple(
                self, console, options, text, code_width, ends_on_nl
            )
            return
        ...
        yield from _syntax_render_lines(
            self, console, options, lines, code_width, line_offset
        )
